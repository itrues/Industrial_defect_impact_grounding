...
class VGDataset(Dataset):
    def __init__(self, data_root, split_root='data', dataset='referit', transforms=[],
                 debug=False, test=False, split='train', max_query_len=128, defined_split='ori',
        ...
        self.test = test
        self.defined_split = defined_split
        self.transforms = []
        ...
        elif self.dataset == 'mvtec_anomaly_detection':
            self.im_dir = '/home/cike/workspace/data/refer/mdetr/mdetr/data/mvtec_anomaly_detection_masked'
        else:  # refer coco etc.
        ...
        if self.dataset == 'mvtec_anomaly_detection':
            #json_name = f'finetune_{self.dataset}_ratio_8_seed_65_mixed_{self.defined_split}_{split}.json'
            json_name = f'finetune_knowledge_mvtec_anomaly_detection_ratio_8_seed_65_{split}.json' 
            with open(osp.join(self.im_dir,json_name),'r') as f:
                json_dict = json.load(f)
                self.images = json_dict['images']
                self.img_box = json_dict['annotations']
        else:
            for split in splits:
        ...
        if self.dataset == 'flickr':
            ...
            self.img_names, self.bboxs, self.phrases = [], [], []
            for ii,jj in zip(self.images, self.img_box):
                self.img_names.append(ii['file_name']) 
                self.phrases.append(ii['caption'])
                self.bboxs.append(jj['bbox'])
        else:
    ...
    def getitem__PIL(self, idx):
        ...
        if 'mask' in target:
            ...
            return image, mask, target, image_path
