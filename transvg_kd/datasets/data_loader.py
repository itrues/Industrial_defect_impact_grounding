...
class TransVGDataset(data.Dataset):
    ...
    def __init__(self, data_root, split_root='data', dataset='referit', 
        ...
        if self.dataset == 'referit':
            ...
            self.split_dir = osp.join(self.dataset_root, 'splits')
        if not self.dataset == 'mvtec_anomaly_detection':
        ...
        if self.dataset == 'mvtec_anomaly_detection':
            #json_name = f'finetune_{self.dataset}_ratio_8_seed_65_mixed_blurred_{split}.json' 
    ...
    def pull_item(self, idx):
        if self.dataset == 'flickr':
            ...
            info_dict = self.images[idx]
            img_file = info_dict['file_name']
            phrase = info_dict['caption']
            bbox = self.img_box[idx]['bbox']
