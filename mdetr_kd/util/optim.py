...
def adjust_learning_rate(
    ...
    if args.schedule == "step":
        ...
        milestones = list(range(args.lr_drop, args.epochs, args.drop_every_steps))
        gamma = 0.5 ** bisect_right(milestones, epoch)
        text_encoder_gamma = gamma
    elif args.schedule == "custom_multistep":
        if type(args.custom_milestones) == list:
            milestones = args.custom_milestones
        else:
            milestones = list(range(args.lr_drop, args.epochs, args.drop_every_steps))
        gamma = 0.5 ** bisect_right(milestones, epoch)
        text_encoder_gamma = gamma
