...
import yaml
from util.optim import update_ema
...
def get_args_parser():
    ...
    parser.add_argument("--reinit_weights_modules", default=["encoder"], type=list, help="text_encoder, backbone, encoder")
    ...
    parser.add_argument("--update_teacher_ema_infer_model_from_model_ema", action="store_true", default=False, help="Whether ")
    ...
    parser.add_argument(
        ...
        type=str,
        choices=("step", "multistep", "linear_with_warmup", "all_linear_with_warmup", "custom_multistep"),
    )
    parser.add_argument("--ema", action="store_true")
    parser.add_argument("--ema_decay", type=float, default=0.9998)
    parser.add_argument("--teacher_ema_decay", type=float, default=0.9998)
    ...
    parser.add_argument("--teacher_model_load", default="", help="resume teacher model from checkpoint")
...
def main(args):
    ...
    if args.load:
        ...
        pretrained_dict = checkpoint["model_ema"] if "model_ema" in checkpoint else checkpoint["model"]
        ...
        """for n in checkpoint["model_ema"]:
            print('ppp', n)
        for n, p in model_without_ddp.transformer.named_parameters():
            print('ppp2', n, p.requires_grad)
        print(args.ema,'pppppppppppppp')"""
        if args.ema:
            model_ema = deepcopy(model_without_ddp)
    ...
    if args.teacher_model_load:
        print("loading teacher model from", args.teacher_model_load)
        checkpoint = torch.load(args.teacher_model_load, map_location="cpu")
        teacher_model = deepcopy(model)
        if args.distributed:
            teacher_model.module.load_state_dict(checkpoint["model"])
            if "model_ema" in checkpoint:
                teacher_model.module.load_state_dict(checkpoint["model_ema"], strict=False)
            else:
                teacher_model.module.load_state_dict(checkpoint["model"], strict=False)
        else:
            if "model_ema" in checkpoint:
                teacher_model.load_state_dict(checkpoint["model_ema"], strict=False)
            else:
        ...
        elif args.update_teacher_ema_infer_model_from_model_ema:
            teacher_model_ema = deepcopy(teacher_model)
        elif args.update_ema_infer_teacher_model:
            teacher_model_ema = teacher_model
    elif args.text_distillation or encoder_distillation or detection_head_distillation:
        teacher_model = deepcopy(model)
        if args.update_ema_teacher_model:
            teacher_model_ema = deepcopy(teacher_model)
        elif args.update_ema_infer_teacher_model:
    ...
    if args.eval:
        ...
        print(log_stats)
        return
    pretrained_backbone_visu = None
    pretrained_text_encoder = None
    pretrained_encoder = None
    if args.load:
        pretrained_backbone_visu = deepcopy(model_without_ddp.backbone.state_dict())
        pretrained_text_encoder = deepcopy(model_without_ddp.transformer.text_encoder.state_dict())
        pretrained_encoder = deepcopy(model_without_ddp.transformer.encoder.state_dict())
    """teacher_model = deepcopy(model)
    teacher_model.to(device)
    teacher_model.eval()"""
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train_stats = train_one_epoch(
            ...
            model_ema=temp_model_ema,
            teacher_model_ema=teacher_model_ema,
            pretrained_dict=pretrained_dict,
        )
        
        
        #if temp_model_ema is not None:
            
        #    update_ema(model, temp_model_ema, args.ema_decay)
        if epoch % args.eval_skip == 0:
            test_stats = {}
            test_model = model #temp_model_ema if temp_model_ema is not None else model
            for i, item in enumerate(val_tuples):
                evaluator_list = build_evaluator_list(item.base_ds, item.dataset_name)
                item = item._replace(evaluator_list=evaluator_list)
                postprocessors = build_postprocessors(args, item.dataset_name)
            ...
            test_stats = {}
        if epoch in args.reinit_weights_epoch:
            reset_weights(model, pretrained_dict, args.reinit_weights_modules)

        if epoch % args.eval_skip == 0 and args.ema:
            test_ema_stats = {}
            if args.accumulate_update_weights:
                test_model = temp_model_ema #temp_model_ema if temp_model_ema is not None else model
            else:
                test_model = model_ema #temp_model_ema if temp_model_ema is not None else model
            for i, item in enumerate(val_tuples):
                evaluator_list = build_evaluator_list(item.base_ds, item.dataset_name)
                item = item._replace(evaluator_list=evaluator_list)
                postprocessors = build_postprocessors(args, item.dataset_name)
                ...
                test_ema_stats.update({item.dataset_name + "_" + k: v for k, v in curr_test_stats.items()})
        else:
            test_ema_stats = {}
        if epoch % args.eval_skip == 0 and teacher_model_ema is not None:
            test_teacher_model_ema_stats = {}
            
            test_model = teacher_model_ema #temp_model_ema if temp_model_ema is not None else model
            for i, item in enumerate(val_tuples):
                evaluator_list = build_evaluator_list(item.base_ds, item.dataset_name)
                item = item._replace(evaluator_list=evaluator_list)
                postprocessors = build_postprocessors(args, item.dataset_name)
                ...
                test_teacher_model_ema_stats.update({item.dataset_name + "_" + k: v for k, v in curr_test_stats.items()})
        else:
            test_teacher_model_ema_stats = {}
        log_stats = {
            ...
            **{f"test_teacher_model_ema_{k}": v for k, v in test_teacher_model_ema_stats.items()},
            "epoch": epoch,
            "n_parameters": n_parameters,
        }
        if 'test_ema_refexp_knowledge_mvtec_anomaly_detection' in log_stats and args.accumulate_update_weights:
            if log_stats['test_ema_refexp_knowledge_mvtec_anomaly_detection'][0] >= best_val_metric:
                msd = temp_model_ema.state_dict()
                for k, ema_v in model_ema.state_dict().items():
                    model_v = msd[k].detach()
                    ema_v.copy_(model_v)
                best_val_metric = log_stats['test_ema_refexp_knowledge_mvtec_anomaly_detection'][0]
                print('\n')
                print('saving best model with val metric: ', best_val_metric)
                print('\n')
            else:
                msd = model_ema.state_dict()
                for k, ema_v in temp_model_ema.state_dict().items():
                    model_v = msd[k].detach()
                    ema_v.copy_(model_v)
                print('\n')
                print('not saving model with val metric: ', log_stats['test_ema_refexp_knowledge_mvtec_anomaly_detection'][0])
                print('\n')
        if args.output_dir and dist.is_main_process():
            with (output_dir / "log.txt").open("a") as f:
                f.write(json.dumps(log_stats) + "\n")
            acc_logs = {}
            if "test_refexp_knowledge_mvtec_anomaly_detection" in log_stats:
                acc_logs['refexp'] = log_stats['test_refexp_knowledge_mvtec_anomaly_detection']
            if 'test_ema_refexp_knowledge_mvtec_anomaly_detection' in log_stats:
                acc_logs['refexp_ema'] = log_stats['test_ema_refexp_knowledge_mvtec_anomaly_detection']
            if 'test_teacher_model_ema_refexp_knowledge_mvtec_anomaly_detection' in log_stats:
                acc_logs['refexp_teacher_ema'] = log_stats['test_teacher_model_ema_refexp_knowledge_mvtec_anomaly_detection']
            with (output_dir / "acc_logs.txt").open("a") as f:
                f.write(json.dumps(acc_logs) + "\n")
        if args.output_dir and dist.is_main_process():
                ...
                dist.save_on_master(
                    {
                        ...
                        "teacher_model_ema": teacher_model_ema.state_dict() if teacher_model_ema is not None else None,
        ...
        if epoch % args.eval_skip == 0 and dist.is_main_process():
            if 'refexp_knowledge_mvtec_anomaly_detection' in test_stats:
                val_metric = test_stats['refexp_knowledge_mvtec_anomaly_detection'][0]
            else:
                val_metric = np.mean([v[1] for k, v in test_stats.items() if "coco_eval_bbox" in k])
            if args.do_qa:
            ...
            if args.output_dir and val_metric > best_metric:
                best_metric = metric
        ...
        """if epoch in args.reinit_weights_epoch and args.load:
            print('\n')
            print('--------------> reinitializing pretrained weights for epoch {} <--------------'.format(epoch))
            print('\n')
            if 'backbone' in args.reinit_weights_modules:
                reset_weights(model.module, checkpoint['model_ema'] if 'model_ema' in checkpoint else checkpoint['model'], 'backbone.')
            if 'text_encoder' in args.reinit_weights_modules:
                reset_weights(model.module, checkpoint['model_ema'] if 'model_ema' in checkpoint else checkpoint['model'], 'transformer.text_encoder.')
            if 'encoder' in args.reinit_weights_modules:
                reset_weights(model.module, checkpoint['model_ema'] if 'model_ema' in checkpoint else checkpoint['model'], 'transformer.encoder.')"""
        """
            if 'backbone' in args.reinit_weights_modules:
                model.module.backbone.load_state_dict(pretrained_backbone_visu)
            if 'text_encoder' in args.reinit_weights_modules:
                model.module.transformer.text_encoder.load_state_dict(pretrained_text_encoder)
            if 'encoder' in args.reinit_weights_modules:
                model.module.transformer.encoder.load_state_dict(pretrained_encoder)"""
    total_time = time.time() - start_time
    total_time_str = str(datetime.timedelta(seconds=int(total_time)))
    print("Training time {}".format(total_time_str))
def reset_weights(model, pretrained_dict, target_moduel_name):
    current_state_dict = {}
    for target_name in target_moduel_name:
        for name, param in pretrained_dict.items():
            if target_name in name:
                current_state_dict[name] = param
    model.module.load_state_dict(current_state_dict,strict=False)
