...
class RefExpEvaluator(object):
    def __init__(self, refexp_gt, iou_types, k=(1, 5, 10), thresh_iou=0.5):
        ...
        self.iou_thrs = [0.5 + 0.05 * i for i in range(0,9)]
    ...
    def summarize(self):
        if dist.is_main_process():
            dataset2score = {
                ...
                "mvtec_anomaly_detection": {k: 0.0 for k in self.k},
                "knowledge_mvtec_anomaly_detection": {k: 0.0 for k in self.k},
            }
            miou = 0.0
            dataset2score_all = {str(k): 0.0 for k in self.iou_thrs}
            dataset2count = {"refcoco": 0.0, "refcoco+": 0.0, "refcocog": 0.0, "mvtec_anomaly_detection": 0.0,"knowledge_mvtec_anomaly_detection": 0.0}
                ...
                for k in self.k:
                    ...
                    if k == 1:
                        ...
                        for iou_thr in self.iou_thrs:
                            if max(giou[:k]) >= iou_thr:
                                dataset2score_all[f"{iou_thr}"] += 1.0
            ...
            for kk, vv in dataset2score_all.items():
                dataset2score_all[kk] = vv / dataset2count[dataset_name]
            ...
            print('  '.join([f'(val acc)',*[f'Acc@{float(k):.2f}: {v:.4f}' for k, v in dataset2score_all.items()],f" Mean_iou: {miou.item() / dataset2count[dataset_name]:.4f}"]))
            return results
        return None
...
def build(image_set, args):
    ...
    if refexp_dataset_name in ['knowledge_mvtec_anomaly_detection_ratio_8_seed_65','mvtec_anomaly_detection','mvtec_anomaly_detection_ratio_8_seed_65', 'mvtec_anomaly_detection_ratio_8_seed_65_blurred','mvtec_anomaly_detection_ratio_8_seed_65_mixed_deblurred',
    'mvtec_anomaly_detection_ratio_8_seed_65_mixed_blurred', 'building_anomaly_detection_ratio_8_seed_65']:
    ...
    elif refexp_dataset_name in ['knowledge_mvtec_anomaly_detection_ratio_8_seed_65','mvtec_anomaly_detection','mvtec_anomaly_detection_ratio_8_seed_65','mvtec_anomaly_detection_ratio_8_seed_65_blurred','mvtec_anomaly_detection_ratio_8_seed_65_mixed_deblurred',
    'mvtec_anomaly_detection_ratio_8_seed_65_mixed_blurred', 'building_anomaly_detection_ratio_8_seed_65']:
        ann_file = Path(args.refexp_ann_path) / f"finetune_{refexp_dataset_name}_{image_set}.json"
    else:
